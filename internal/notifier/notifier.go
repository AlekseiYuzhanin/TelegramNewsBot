...
type Summarizer interface {
	Summarize(ctx context.Context, test string) (string,error)
}
...
func (n *Notifier) sendArticle(article model.Article, summary string) error {
	...
	msg := tgbotapi.NewMessage(n.channelID, fmt.Sprintf(
		msgFormat, markup.EscapeFromMarkdown(article.Title), 
		markup.EscapeFromMarkdown(summary), 
		markup.EscapeFromMarkdown(article.Link)))
...
var redunantNewLines = regexp.MustCompile(`\n{3,}`)

func cleanText(text string) string {
	return redunantNewLines.ReplaceAllString(text, "\n")
}